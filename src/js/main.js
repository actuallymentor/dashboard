...
window.onload = f => {
	...
	q( "input[type='text']" ).addEventListener( 'keyup', event => event.target.style.width = `${ event.target.value.length }rem` )
	...
	q( '#mit' ).addEventListener( 'keyup', event => Cookie.set( 'mit', event.target.value, { expires: midnight } ) )
	...
	q( '#lit' ).addEventListener( 'keyup', event => Cookie.set( 'lit', event.target.value, { expires: midnight } ) )
